...
foreach ($items as $item) {
    ...
    $message .= "  (Foto: " . $base_url . $item['image'] . " )\n";
}
...
<div class="bg-gray-50 min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 text-center">
            <?php 
            // Fetch Payment Method Status for Header
            $stmt_pm_check = $pdo->prepare("SELECT name, requires_proof FROM payment_methods WHERE code = ?");
            $stmt_pm_check->execute([$order['payment_method']]);
            $pm_check = $stmt_pm_check->fetch();
            $needs_proof = $pm_check['requires_proof'] ?? 0;

            $is_pending_payment = $needs_proof && empty($order['proof_of_payment']);
            ?>
            ...
            $stmt_pm = $pdo->prepare("SELECT * FROM payment_methods WHERE code = ?");
            $stmt_pm->execute([$order['payment_method']]);
            $pm_details = $stmt_pm->fetch();
            
            $requires_proof = $pm_details['requires_proof'] ?? 0;
            ?>

            <?php if ($pm_details): ?>
                <div class="mt-6 bg-white border border-gray-200 rounded-md p-4 text-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900 mb-2"><?php echo htmlspecialchars($pm_details['name']); ?></h3>
                    
                    <?php if ($pm_details['image']): ?>
                        <img src="<?php echo htmlspecialchars($pm_details['image']); ?>" alt="<?php echo htmlspecialchars($pm_details['name']); ?>" class="mx-auto h-48 w-48 object-contain border mb-4">
                    <?php endif; ?>
                    
                    <div class="text-sm text-gray-700 bg-gray-50 p-3 rounded">
                        <?php echo nl2br(htmlspecialchars($pm_details['description'])); ?>
                    </div>
                </div>
                ...
                <?php else: ?>
                    <div class="mt-6 text-center">
                        <p class="text-gray-600 mb-4">Pesanan Anda sedang diproses. Mohon tunggu konfirmasi.</p>
            ...
            <?php else: ?>
                <!-- Fallback if payment method not found in DB -->
                <div class="mt-6 bg-red-50 border border-red-200 rounded-md p-4">
                    <p class="text-red-800">Metode pembayaran tidak ditemukan. Silakan hubungi admin.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
